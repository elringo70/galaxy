...
const texture = new THREE.TextureLoader().load("./resources/whiteDot.png");
...
export class Star {
  ...
  updateScale(camera) {
    let distance = this.position.distanceTo(camera.position) / 250;

    let starSize = distance * starTypes.size[this.starType];
    starSize = clamp(starSize, STAR_MIN, STAR_MAX);
    this.object?.scale.copy(new THREE.Vector3(starSize, starSize, starSize));
  }
